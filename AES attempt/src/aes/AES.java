package aes;

public class AES {

	private static int numberOfRounds,numKeyLength,numOfCols;

	private static byte[][]subKey;

	private static final int[] S_BOX = {
			 
	};

	private static final int RCON[] = { 
			 	
	};

	public AES(){
		// this will be the 128 bit version of the AES, so 16 bytes and 10 rounds
	}

	public static byte[] AESEncrypt(byte[] input,byte[] key){
		numOfCols = 4;
		numKeyLength = key.length/4;
		numberOfRounds = numKeyLength+6;

		return null;

	}
	private static byte[] xorAdd(byte[] a, byte[] b){
		byte[] output = new byte[a.length];
		for(int i = 0; i < a.length; i++){
			output[i] = (byte) (a[i]^b[i]);//xoring the two bytes returns an int
		}
		return output;
	}

	private static byte[][] generateSubKeys(byte[] key){
		//since AES expands the original key for each round, we need to build the subkeys
		byte[][]temp = new byte[numOfCols*(numberOfRounds+1)][4];
		int i = 0;
		while(i < numKeyLength){
			temp[i][0] = key[i * 4];
			temp[i][1] = key[i * 4 + 1];
			temp[i][2] = key[i * 4 + 2];
			temp[i][3] = key[i * 4 + 3];
			i++;
		}

		while(i<numOfCols*(numberOfRounds+1)){
			byte[]tmp = new byte[4];
			for(int k = 0;k < 4; k++){
				tmp[k] = temp[i-1][k];
				if(i%numKeyLength==0){
					tmp = subWord(rotateWord(tmp));
					tmp[0] = (byte) (tmp[0] ^ (RCON[i / numKeyLength] & 0xff));
				}
				else if (numKeyLength > 6 && i % numKeyLength == 4){
					tmp = subWord(tmp);
				}
				temp[i] = xorAdd(temp[i-numKeyLength],tmp);
				i++;
			}
		}

		return temp;

	}

	private static byte[] subWord(byte[] in) {
		byte[] temp = new byte[in.length];

		for(int i = 0; i < temp.length; i++){
			temp[i] = (byte) (S_BOX[in[i]& 0x000000ff]& 0xff);
		}
		return temp;
	}

	private static byte[] rotateWord(byte[] in) {
		byte[] tmp = new byte[in.length];
		tmp[0] = in[1];
		tmp[1] = in[2];
		tmp[2] = in[3];
		tmp[3] = in[0];

		return tmp;
	}

	private static byte[][] AddRoundKey(byte[][] state, byte[][] w, int round) {

		byte[][] tmp = new byte[state.length][state[0].length];

		for (int c = 0; c < numOfCols; c++) {
			for (int l = 0; l < 4; l++)
				tmp[c][l] = (byte) (state[round * numOfCols + c][l] ^ w[round * numOfCols + c][l]);
		}

		return tmp;
	}
	
	private static byte[][]subBytes(byte[][] state){
		byte[][] temp = new byte[state.length][state[0].length];
		for(int i = 0; i < 4; i++){
			for(int j = 0; j < numOfCols;j++){
				temp[i][j] = (byte) S_BOX[state[i][j]& 0x000000ff];//substitutes values for S_BOX equivalent
			}
		}
		
		return temp;
	}

	private static byte[][] shiftRows(byte[][]state){
		byte[] test = new byte[state[0].length];
		for(int i = 1; i < 4; i++){
			for(int j = 0; j < numOfCols; j++){
				test[j] = state[i][(i+j)%numOfCols];
			}
			state[i] = test;
		}
		return state;
	}
	
	private static byte[][]mixColumns(byte[][] state){
		int[] test = new int[4];
		byte x02 = (byte)0x02;
		byte x03 = (byte)0x03;
		for(int i = 0; i < 4; i++){
			test[0] = FFMul(x02, state[i][0]) ^ FFMul(x03, state[i][1]) ^ state[i][2] ^ state[i][3];
			 test[1] = state[i][0] ^ FFMul(x02, state[i][1]) ^ FFMul(x03, state[i][2]) ^ state[i][3];
			 test[2] = state[i][0] ^ state[i][1] ^ FFMul(x02, state[i][2]) ^ FFMul(x03, state[i][3]);
			 test[3] = FFMul(x03, state[i][0]) ^ state[i][1] ^ state[i][2] ^ FFMul(x02, state[i][3]);
			 for (int k = 0; k < 4; k++) state[i][k] = (byte)(test[k]);
		}
		return state;
	}

	private static int FFMul(byte a, byte b) {

		return 0;
	}















}
